...
llama_model_qwen35::graph_mtp::graph_mtp(const llama_model & model, const llm_graph_params & params)
    ...
    if (cparams.mtp_chain && n_tokens > 1 && ubatch.n_seqs_unq == 1 && ubatch.token) {
        ...
        for (int64_t row = 0; row < n_tokens; ++row) {
            ...
            ggml_tensor * id;
            ggml_tensor * packed;
            if (cparams.mtp_chain_sharded_head) {
                ggml_tensor * stats = ggml_tp_top1_stats(
                        ctx0, logits, 0, 2, 0, cparams.mtp_chain_need_probability);
                packed = ggml_tp_top1_select(ctx0, stats, 2, cparams.mtp_chain_need_probability);
                id = ggml_cast(ctx0, ggml_view_1d(ctx0, packed, 1, 0), GGML_TYPE_I32);
            } else {
                id = ggml_argmax(ctx0, logits);
                ggml_tensor * value;
                ...
                packed = ggml_concat(ctx0, id_f, value, 0);
            }
