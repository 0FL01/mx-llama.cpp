...
static void test_batch(int nt, ggml_type type, ggml_backend_t device) {
    ...
    c.observations = ggml_new_tensor_1d(ctx, GGML_TYPE_I64, 5);
    ...
    for (int round = 0; round < 4; ++round) {
        ...
        const auto chain = [&](ggml_tensor * u, ggml_tensor * g, ggml_tensor * d, ggml_tensor * route, bool skip) {
            auto * up = ggml_mul_mat_id(gctx, u, input, route);
            auto * gate = ggml_mul_mat_id(gctx, g, input, route);
        ...
        auto * ref = chain(c.up_src, c.gate_src, c.down_src, ids, false);
        auto * cpu = chain(c.up_src, c.gate_src, c.down_src, ids, true);
        auto * hit = chain(c.up_c, c.gate_c, c.down_c, slots, false);
        ...
        for (int t = 0; t < nt; ++t) {
            for (int i = 0; i < 4; ++i) { route[t*4+i] = (i + (round == 3 ? 3 : round == 2 ? 2 : 0)) % 4; }
        }
