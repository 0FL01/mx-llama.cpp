...
// gfx906 DPP-based warp reductions.
//
...
//
// The s_nop prefixes are mandatory VALU-to-DPP hazard wait states. The
// compiler inserts them for DPP it emits itself but cannot see data
// dependencies through asm block boundaries.

#define GGML_GFX906_DPP_F32(name, barrier, dpp_ctrl, vop_instr)               \
...
GGML_GFX906_DPP_F32(ggml_gfx906_add_xor1_f32, "s_nop 4\n", "quad_perm:[1,0,3,2]", "v_add_f32_dpp")
GGML_GFX906_DPP_F32(ggml_gfx906_add_xor2_f32, "s_nop 1\n", "quad_perm:[2,3,0,1]", "v_add_f32_dpp")
GGML_GFX906_DPP_F32(ggml_gfx906_add_xor8_f32, "s_nop 1\n", "row_ror:8",           "v_add_f32_dpp")
GGML_GFX906_DPP_F32(ggml_gfx906_max_xor1_f32, "s_nop 4\n", "quad_perm:[1,0,3,2]", "v_max_f32_dpp")
GGML_GFX906_DPP_F32(ggml_gfx906_max_xor2_f32, "s_nop 1\n", "quad_perm:[2,3,0,1]", "v_max_f32_dpp")
GGML_GFX906_DPP_F32(ggml_gfx906_max_xor8_f32, "s_nop 1\n", "row_ror:8",           "v_max_f32_dpp")

#undef GGML_GFX906_DPP_F32

template <int offset>
static __device__ __forceinline__ int ggml_gfx906_xfer_xor_i32(const int x) {
    int d;
    if constexpr (offset == 1) {
        ...
        static_assert(offset == 16, "unsupported xor offset");
...
static __device__ __forceinline__ float ggml_gfx906_xfer_xor4_f32(const float x) {
    return __int_as_float(ggml_gfx906_xfer_xor_i32<4>(__float_as_int(x)));
}

static __device__ __forceinline__ float ggml_gfx906_xfer_xor16_f32(const float x) {
    return __int_as_float(ggml_gfx906_xfer_xor_i32<16>(__float_as_int(x)));
}

template <int width>
static __device__ __forceinline__ float ggml_gfx906_warp_reduce_sum_f32(float x) {
    static_assert(width >= 1 && (width & (width - 1)) == 0, "width must be a power of 2");
    if constexpr (width >=  2) { x = ggml_gfx906_add_xor1_f32(x); }
    if constexpr (width >=  4) { x = ggml_gfx906_add_xor2_f32(x); }
    if constexpr (width >=  8) { x += ggml_gfx906_xfer_xor4_f32(x); }
    if constexpr (width >= 16) { x = ggml_gfx906_add_xor8_f32(x); }
    if constexpr (width >= 32) { x += ggml_gfx906_xfer_xor16_f32(x); }
    if constexpr (width >= 64) { x += __shfl_xor_sync(0xffffffff, x, 32, 64); }
    return x;
}

template <int width>
static __device__ __forceinline__ float ggml_gfx906_warp_reduce_max_f32(float x) {
    static_assert(width >= 1 && (width & (width - 1)) == 0, "width must be a power of 2");
    if constexpr (width >=  2) { x = ggml_gfx906_max_xor1_f32(x); }
    if constexpr (width >=  4) { x = ggml_gfx906_max_xor2_f32(x); }
    if constexpr (width >=  8) { x = fmaxf(x, ggml_gfx906_xfer_xor4_f32(x)); }
    if constexpr (width >= 16) { x = ggml_gfx906_max_xor8_f32(x); }
    if constexpr (width >= 32) { x = fmaxf(x, ggml_gfx906_xfer_xor16_f32(x)); }
    if constexpr (width >= 64) { x = fmaxf(x, __shfl_xor_sync(0xffffffff, x, 32, 64)); }
    return x;
}

template <int width>
static __device__ __forceinline__ int ggml_gfx906_warp_reduce_sum_i32(int x) {
    static_assert(width >= 1 && (width & (width - 1)) == 0, "width must be a power of 2");
    if constexpr (width >=  2) { x += ggml_gfx906_xfer_xor_i32<1>(x); }
    if constexpr (width >=  4) { x += ggml_gfx906_xfer_xor_i32<2>(x); }
    if constexpr (width >=  8) { x += ggml_gfx906_xfer_xor_i32<4>(x); }
    if constexpr (width >= 16) { x += ggml_gfx906_xfer_xor_i32<8>(x); }
    if constexpr (width >= 32) { x += ggml_gfx906_xfer_xor_i32<16>(x); }
    if constexpr (width >= 64) { x += __shfl_xor_sync(0xffffffff, x, 32, 64); }
    return x;
}
...
#if defined(GGML_USE_HIP) && defined(__gfx906__)
    return ggml_gfx906_warp_reduce_sum_i32<width>(x);
...
#if defined(GGML_USE_HIP) && defined(__gfx906__)
    return ggml_gfx906_warp_reduce_sum_f32<width>(x);
#else
#pragma unroll
    for (int offset = width/2; offset > 0; offset >>= 1) {
        x += __shfl_xor_sync(0xffffffff, x, offset, width);
...
#if defined(GGML_USE_HIP) && defined(__gfx906__)
    a.x = ggml_gfx906_warp_reduce_sum_f32<width>(a.x);
    a.y = ggml_gfx906_warp_reduce_sum_f32<width>(a.y);
    return a;
#else
#pragma unroll
    for (int offset = width/2; offset > 0; offset >>= 1) {
...
#if defined(GGML_USE_HIP) && defined(__gfx906__)
    return ggml_gfx906_warp_reduce_max_f32<width>(x);
#else
#pragma unroll
    for (int offset = width/2; offset > 0; offset >>= 1) {
