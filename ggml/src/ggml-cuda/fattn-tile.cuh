...
static __device__ __forceinline__ void flash_attn_tile_load_tile_q4_0(
        const block_q4_0 * const __restrict__ KV, half2 * const __restrict__ tile_KV,
        const int stride_KV, const int i_sup) {
    static_assert(J % QK4_0 == 0, "bad J");

    constexpr int blocks_per_row = J/QK4_0;
    constexpr int nblocks = I*blocks_per_row;
...
#pragma unroll
    for (int b0 = 0; b0 < nblocks; b0 += nwarps*warp_size) {
        const int b = b0 + tid;
        if (b < nblocks) {
            const int i = b / blocks_per_row;
            const int j = b % blocks_per_row;
            half2 * tile = tile_KV + i*(J/2 + J_padding) + j*(QK4_0/2);

            if (!oob_check || i < i_sup) {
                const block_q4_0 * GGML_CUDA_RESTRICT block = KV + int64_t(i)*stride_KV + j;
                const float d = __half2float(block->d);
                const float dm = -8.0f*d;
                const uint16_t * GGML_CUDA_RESTRICT qs = (const uint16_t *) block->qs;

#pragma unroll
                for (int l = 0; l < QK4_0/4; ++l) {
                    const uint16_t q = qs[l];
                    tile[l] = make_half2(
                        d*int(q & 0x000F) + dm,
                        d*int((q >> 8) & 0x000F) + dm);
                    tile[QK4_0/4 + l] = make_half2(
                        d*int((q >> 4) & 0x000F) + dm,
                        d*int((q >> 12) & 0x000F) + dm);
                }
            } else {
...
template <int warp_size, int nwarps, int ncols1, int ncols2, int DKQ, int nbatch_fa, int nbatch_K,
    bool use_logit_softcap, bool oob_check, typename T_vec_dot, bool kv_q4_0 = false>
static __device__ __forceinline__ void flash_attn_tile_iter_KQ(
        T_vec_dot   * const Q_tmp,
        const half2 * const __restrict__ K_h2,
        const block_q4_0 * const __restrict__ K_q4,
        T_vec_dot   * const KV_tmp,
        const int stride_K2,
        const int stride_K_q4,
        const int k_VKQ_0,
        const int k_VKQ_sup,
        const int k_KQ_0,
        float * KQ_acc) {
    ...
    if constexpr (kv_q4_0) {
        flash_attn_tile_load_tile_q4_0<warp_size, nwarps, nbatch_fa, nbatch_K, cpy_ne, oob_check>
            (K_q4 + int64_t(k_VKQ_0)*stride_K_q4 + k_KQ_0/QK4_0, KV_tmp, stride_K_q4, k_VKQ_sup);
    } else {
...
#pragma unroll
    for (int k_KQ_0 = 0; k_KQ_0 < DKQ - nbatch_K_last; k_KQ_0 += nbatch_K) {
        flash_attn_tile_iter_KQ<warp_size, nwarps, ncols1, ncols2, DKQ, nbatch_fa, nbatch_K, use_logit_softcap, oob_check, T_vec_dot, kv_q4_0>(
            Q_tmp, K_h2, K_q4, KV_tmp, stride_K2, stride_K_q4, k_VKQ_0, k_VKQ_sup, k_KQ_0, KQ_acc);
    }
    if constexpr (nbatch_K_last > 0) {
        constexpr int k_KQ_0 = DKQ - nbatch_K_last;
        flash_attn_tile_iter_KQ<warp_size, nwarps, ncols1, ncols2, DKQ, nbatch_fa, nbatch_K_last, use_logit_softcap, oob_check, T_vec_dot, kv_q4_0>(
            Q_tmp, K_h2, K_q4, KV_tmp, stride_K2, stride_K_q4, k_VKQ_0, k_VKQ_sup, k_KQ_0, KQ_acc);
    }
...
static __global__ void flash_attn_tile(
    ...
    const block_q4_0 * K_q4 = nullptr;
    const block_q4_0 * V_q4 = nullptr;
    if constexpr (kv_q4_0) {
        K_q4 = (const block_q4_0 *) (K + nb13*sequence + nb12*(head0 / gqa_ratio));
    ...
    const int stride_K_q4 = nb11 / sizeof(block_q4_0);
    ...
    if (ncols2 == 1) {
        ...
        while (k_VKQ_0 < k_VKQ_max - nbatch_fa) {
            ...
            flash_attn_tile_iter<warp_size, nwarps, ncols1, ncols2, DKQ, DV, nbatch_fa, nbatch_K, use_logit_softcap, oob_check, kv_q4_0>
                (Q_tmp, K_h2, V_h2, K_q4, V_q4, maskh, ne01, logit_softcap, slope, KQ, KV_tmp,
                stride_K2, stride_V2, stride_K_q4, stride_V_q4, stride_mask, KQ_max, KQ_sum, VKQ, k_VKQ_0, k_VKQ_max, col_Q_0);
...
#ifdef GGML_USE_HIP
    if constexpr (DKQ == 256 && DV == 256 && ncols2 == 2 && !use_logit_softcap) {
        if (ggml_cuda_fattn_tile_q4_0_native(id, dst)) {
