...
// Adapted from upstream f8dbcd618 (ROCm radix TOP_K). Keep the existing
// small-k and hierarchical paths; use radix when their merge cannot fit LDS.
...
static void top_k_radix_cuda(
    ...
    for (int shift = 32 - RADIX_BITS; shift >= 0; shift -= RADIX_BITS) {
        top_k_radix_histogram<BLOCK_SIZE, RADIX_BITS>
            <<<row_grid, BLOCK_SIZE, 0, stream>>>(src, states, histograms, ncols, blocks_per_row, shift);
...
#if defined(GGML_USE_HIP)
    } else if (((ncols + 16383) / 16384) * k > 16384) {
